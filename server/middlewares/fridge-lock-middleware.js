const {tryAcquireFridgeLock, lockLostHandling} = require("../services/fridge-lock-service");
...
const tryAcquireFridgeLockMiddleware = async (req, res, next) => { 
    ...
    try {
        ...
        if (!lockIdentifier) {
            ...
            return res.status(423).json({ error: `Fridge: ${fridge.name} is currently locked, please try again later` });
        }
        ...
        console.log(`Fridge lock acquired for fridgeId: ${fridgeId} by user: ${req.user.name}`);
        return next();
    } catch (error) {
