...
const postGenerateRecipe = async (req, res) => {
  try {
    ...
    if (!ingredients || ingredients.length === 0) {
      return res
        .status(400)
        .json({ error: "No ingredients available for recipe generation" });
    }
    ...
    res
      .status(202)
      .json({ message: "Recipe generation in progress...Waiting..." });
  } catch (error) {
...
const getRecipeResult = async (req, res) => {
  ...
  try {
    ...
    if (llmTask.user_id && llmTask.user_id !== userId) {
      console.error(
        `Unauthorized access attempt by user ${userId} for recipe with traceId ${traceId}`
      );
      return res
        .status(403)
        .json({ error: "Unauthorized access to this recipe" });
    }
    ...
    if (llmTask.fridge_id && fridges.length == 0) {
      ...
      return res
        .status(403)
        .json({ error: "Unauthorized access to this fridge" });
    }
